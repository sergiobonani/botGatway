...
async function logarSistema(parameters) {
    ...
    const retornoAPI = await doRequest(action, { senha, cpf }, true)
    const response = {}

    if (retornoAPI && !retornoAPI.success) {
        if (retornoAPI.details['CpfCnpj'] && retornoAPI.details['CpfCnpj'].length >= 0) {
            response.mensagem = 'O CPF informado está inválido. Digite INFORMAR CPF, para informar novamente.'
        }
        else if (retornoAPI.details['Senha'] && retornoAPI.details['Senha'].length >= 0) {
...
async function doRequest(action, parameters, isFormData) {
    const res = await fetch(`${process.env.API_CONSORCIO_URL}/${action.endpoint}`, {
        method: 'POST',
        headers: buildHeaders(),
        body: buildBody(parameters, action.parametros, isFormData)
    })
...
function buildBody(parameters, parametersName, isFormData) {
    const body = {}
